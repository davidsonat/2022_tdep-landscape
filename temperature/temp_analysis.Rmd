...
cv.plot <- ggplot(temp.df, aes(x = temp_range)) +
  geom_histogram(binwidth = 2, colour = "black", fill = "grey80") +  
  theme_classic( base_size = 24, base_line_size = 1 ) + 
  scale_y_continuous( expand = c(0, 0) ) +
  xlab("Daily Temp Range (°C)") +
  theme( panel.border = element_rect( color = "black", fill = NA, size = 0.75 ),
         legend.position = "none",
         aspect.ratio = 1)
...
# ## Without Pool ID random effect
# # Depth, Area, Shade, and River Temp 
# temp.cand.models[[16]] <- glmmTMB(MaxWTemp_C ~ Depth_Z * SurfArea_Z * Pct_Shaded_Z + MaxRTemp_Z, data = temp.df )
# 
# # Depth, Area, Shade
# temp.cand.models[[17]] <- glmmTMB(MaxWTemp_C ~ Depth_Z * SurfArea_Z * Pct_Shaded_Z, data = temp.df )
# 
# # Depth, Area, River Temp
# temp.cand.models[[18]] <- glmmTMB(MaxWTemp_C ~ Depth_Z * SurfArea_Z + MaxRTemp_Z, data = temp.df )
# 
# # Depth, Shade, River Temp
# temp.cand.models[[19]] <- glmmTMB(MaxWTemp_C ~ Depth_Z * Pct_Shaded_Z + MaxRTemp_Z, data = temp.df )
# 
# # Area, Shade, River Temp
# temp.cand.models[[20]] <- glmmTMB(MaxWTemp_C ~ SurfArea_Z * Pct_Shaded_Z + MaxRTemp_Z, data = temp.df )
# 
# # Depth and Area
# temp.cand.models[[21]] <- glmmTMB(MaxWTemp_C ~ Depth_Z * SurfArea_Z, data = temp.df )
# 
# # Depth and Shade
# temp.cand.models[[22]] <- glmmTMB(MaxWTemp_C ~ Depth_Z * Pct_Shaded_Z, data = temp.df )
# 
# # Depth and River Temp
# temp.cand.models[[23]] <- glmmTMB(MaxWTemp_C ~ Depth_Z + MaxRTemp_Z, data = temp.df )
# 
# # Area and Shade
# temp.cand.models[[24]] <- glmmTMB(MaxWTemp_C ~ SurfArea_Z * Pct_Shaded_Z, data = temp.df )
# 
# # Area and River Temp
# temp.cand.models[[25]] <- glmmTMB(MaxWTemp_C ~ SurfArea_Z + MaxRTemp_Z, data = temp.df )
# 
# # Shade and River Temp
# temp.cand.models[[26]] <- glmmTMB(MaxWTemp_C ~ Pct_Shaded_Z + MaxRTemp_Z, data = temp.df )
# 
# # Depth alone
# temp.cand.models[[27]] <- glmmTMB(MaxWTemp_C ~ Depth_Z, data = temp.df )
# 
# # Area alone
# temp.cand.models[[28]] <- glmmTMB(MaxWTemp_C ~ SurfArea_Z, data = temp.df )
# 
# # Shade alone
# temp.cand.models[[29]] <- glmmTMB(MaxWTemp_C ~ Pct_Shaded_Z, data = temp.df )
# 
# # River temp alone
# temp.cand.models[[30]] <- glmmTMB(MaxWTemp_C ~ MaxRTemp_Z, data = temp.df )
```
...
aic_table <- aictab(cand.set = temp.cand.models, modnames = unlist(mod.names), sort = TRUE)
