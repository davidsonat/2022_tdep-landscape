...
Lastly, let's try simplifying things and instead of using lat/long, let's use a slab-level variable as a random effect. This might allow for more accurate predictions, because we know what the levels of that random effect should be.
...
Nope, this doesn't even really improve marginal r^2^.
...
testdata.df$MaxRTemp_C <- rep(25, nrow(testdata.df)) # It actually doesn't seem to matter what value you put here, because it looks for it but doesn't use it lol
...
=======
Awesome. The bad news is that we can't predict random effects outside of the levels we've observed. The predictions above are "population-level" estimates, meaning that during the time period we collected data on, that's approximately what each pool's temperature would be on average. This may be the best we can do with the data we have, but I think I am comfortable treating it as an index of relative pool temperature.

```{r, eval = FALSE, echo = FALSE}
temp_vec <- seq(20, 30, by = 0.5)
>>>>>>> 117ab8ba7cda0b9ed291abd6d2add6b72064c90d
