...
abundance.df$drgn_presence <- ifelse(abundance.df$Skimmer_Dragonfly > 0, 'yes', 'no')
abundance.df$drgn_presence <- as.factor(abundance.df$drgn_presence)
abundance.df$rel_temp <- ifelse(abundance.df$Temp_Zscore > 0, 'high', 'low')
...
ggplot(data = abundance3.df, aes(y = Mosquito, x = drgn_presence, colour = rel_temp)) +
...
# Temperature alone
mosq.cand.models[[1]] <- glmmTMB(Mosquito ~ Temp_Zscore + (1 | Pool_ID), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)

# Dragons alone
mosq.cand.models[[2]] <- glmmTMB(Mosquito ~ Skimmer_Z + (1 | Pool_ID), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)

# Days since last flooding alone
mosq.cand.models[[3]] <- glmmTMB(Mosquito ~ Days_Between_Z + (1 | Pool_ID), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)

# Temperature and dragons
mosq.cand.models[[4]] <- glmmTMB(Mosquito ~ Temp_Zscore * Skimmer_Z + (1 | Pool_ID), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)

# Temperature and days since
mosq.cand.models[[5]] <- glmmTMB(Mosquito ~ Temp_Zscore + Days_Between_Z + (1 | Pool_ID), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)

# Dragons and days since
mosq.cand.models[[6]] <- glmmTMB(Mosquito ~ Skimmer_Z + Days_Between_Z + (1 | Pool_ID), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)

# All three
mosq.cand.models[[7]] <- glmmTMB(Mosquito ~ Temp_Zscore * Skimmer_Z + Days_Between_Z + (1 | Pool_ID), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)
...
plot1.df <- plot1.df[-c(9,10),]
dwplot(plot1.df, 
       vline = geom_vline(xintercept = 0, colour = "red", linetype = 2), 
       dot_args = list(size = 4), 
       line_args = list(size = 6)) + 
  theme_classic(base_size = 16) 
effect_plot(mosq.cand.models[[4]], pred = Temp_Zscore, interval = TRUE, plot.points = TRUE) 
effect_plot(mosq.cand.models[[4]], pred = Skimmer_Z, interval = TRUE, plot.points = TRUE)
```
...
```{r}
plot2.df <- broom::tidy(dfly.cand.models[[2]])
plot2.df$model <- c("nbinom", "nbinom", "zi", "zi", "cond", "zi")
plot2.df <- plot2.df[-c(5,6),]
dwplot(plot2.df, 
       vline = geom_vline(xintercept = 0, colour = "red", linetype = 2), 
       dot_args = list(size = 4), 
       line_args = list(size = 6)) + 
  theme_classic(base_size = 16) 
effect_plot(dfly.cand.models[[2]], pred = Pct_Shaded_Z, interval = TRUE, plot.points = TRUE) 

plot3.df <- broom::tidy(dfly.cand.models[[6]])
plot3.df$model <- c("nbinom", "nbinom", "nbinom", "zi", "zi", "zi", "cond", "zi")
plot3.df <- plot3.df[-c(7,8),]
dwplot(plot3.df, 
       vline = geom_vline(xintercept = 0, colour = "red", linetype = 2), 
       dot_args = list(size = 4), 
       line_args = list(size = 6)) + 
  theme_classic(base_size = 16) 
effect_plot(dfly.cand.models[[6]], pred = Pct_Shaded_Z, interval = TRUE, plot.points = TRUE) 
```