...
coeff.plot1 <- dwplot(plot1.df, 
       vline = geom_vline(xintercept = 0, colour = "grey80", linetype = 2, size = 1), 
       dot_args = list(size = 4), 
       line_args = list(size = 6)) + 
  theme_classic( base_size = 16, base_line_size = 1 ) + 
  xlab("Coefficient Estimate") +
  scale_color_discrete(name = "Model", labels = c("Zero-Inflation", "Conditional")) +
  theme( panel.border = element_rect( color = "black", fill = NA, size = 0.75 ),
         aspect.ratio = 1,
         axis.title.x = element_text( color = "white" ))


mosq_effect.1 <- effect_plot(mosq.cand.models[[4]], pred = Temp_Zscore, interval = TRUE, plot.points = TRUE, y.label = "Mosquito Abundance", x.label = "Max Temperature") 
mosq_effect.2 <- effect_plot(mosq.cand.models[[4]], pred = Skimmer_Z, interval = TRUE, plot.points = TRUE, y.label = "", x.label = "Dragonfly Abundance")
```
...
plot3.df$term <- c("Shading", "Days Between", "Shading", "Days Between")
coeff.plot2 <- dwplot(plot3.df, 
       vline = geom_vline(xintercept = 0, colour = "grey80", linetype = 2, size = 1), 
       dot_args = list(size = 4), 
       line_args = list(size = 6)) + 
  theme_classic( base_size = 16, base_line_size = 1 ) + 
