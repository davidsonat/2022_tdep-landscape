...
library(performance)
r2(mosq.cand.models[[11]]) # marginal r2 = 0.968, how is that even possible?
...
mosq_effect.1 <- effect_plot(mosq.cand.models[[11]], pred = MaxTemp_Z, interval = TRUE, plot.points = TRUE, y.label = "Mosquito Abundance", x.label = "Max Temperature (°C)", point.size = 1) +  
...
mosq_effect.2 <- effect_plot(mosq.cand.models[[11]], pred = Skimmer_Z, interval = TRUE, plot.points = TRUE, y.label = "Mosquito Abundance", x.label = "Dragonfly Abundance", point.size = 1) +  
  theme_classic( base_size = 12, base_line_size = 0.1 ) +
  scale_x_continuous( expand = c(0, 0), 
...
mosq_effect.plot <- plot_grid( mosq_effect.1, mosq_effect.2, mosq_effect.3,
                        labels = c("a)", "b)", "c)"),
                        nrow = 3,
                        align = "hv" )


ggsave( "effectplot.png",
