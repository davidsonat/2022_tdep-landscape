...
mod.names <- list()
for (i in 1:length(mosq.cand.models)){
mod.names[i] <- as.character(formula(mosq.cand.models[[i]], component = c("cond")))
}
aic_table <- aictab(cand.set = mosq.cand.models, modnames = unlist(mod.names), sort = TRUE)
...
# Temperature alone
dfly.cand.models[[1]] <- glmmTMB(Skimmer_Dragonfly ~ Temp_Zscore + (1 | Date), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)
...
dfly.cand.models[[4]] <- glmmTMB(Skimmer_Dragonfly ~ Temp_Zscore + Pct_Shaded_Z + (1 | Date), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)

# Temperature and days since
dfly.cand.models[[5]] <- glmmTMB(Skimmer_Dragonfly ~ Temp_Zscore + Days_Between_Z + (1 | Date), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)
...
# All three
dfly.cand.models[[7]] <- glmmTMB(Skimmer_Dragonfly ~ Temp_Zscore + Pct_Shaded_Z + Days_Between_Z + (1 | Date), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)

mod.names <- list()
for (i in 1:length(dfly.cand.models)){
mod.names[i] <- as.character(formula(dfly.cand.models[[i]], component = c("cond")))
}
aic_table <- aictab(cand.set = dfly.cand.models, modnames = unlist(mod.names), sort = TRUE)
write.csv(aic_table, file = "../abundance/drgn_aic_table.csv")
