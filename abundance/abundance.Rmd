...
# int_effect3$drgn_presence <- int_effect3$group
# int_effect.plot3 <- ggplot( int_effect3, aes( MaxTemp, predicted + 1, colour = drgn_presence ) ) +
#   geom_line() +
#   geom_ribbon(aes( ymin = conf.low + 1, ymax = conf.high + 1, fill = drgn_presence ), alpha = 0.25, show.legend = F, colour = NA) +
...
ambient.df <- predict(mosq.cand.models[[4]], newdata = abundance.df, type = "response", allow.new.levels = TRUE, se.fit = TRUE)
sum(ambient.df$fit) # mean estimate = 8593.3
sum(ambient.df$fit - ambient.df$se.fit) # low end = 4136.3
sum(ambient.df$fit + ambient.df$se.fit) # high end = 13050.3

abundance2.df$MaxTemp_Z <- abundance.df$MaxTemp_Z + (2.5)/sd(abundance.df$MaxTemp_Predicted_C)
warmed.df <- predict(mosq.cand.models[[4]], newdata = abundance2.df, type = "response", allow.new.levels = TRUE, se.fit = TRUE)
sum(warmed.df$fit) # mean estimate = 15466.23
sum(warmed.df$fit - warmed.df$se.fit) # low end = 6212.8
sum(warmed.df$fit + warmed.df$se.fit) # high end = 24679.7
