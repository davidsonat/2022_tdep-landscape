...
int_effect$Skimmer_Dragonfly <- as.numeric(as.character(int_effect$group)) * sd(abundance.df$Skimmer_Dragonfly) + mean(abundance.df$Skimmer_Dragonfly)
int_effect$Skimmer_Dragonfly <- round(int_effect$Skimmer_Dragonfly, digits = 0)
int_effect$Skimmer_Dragonfly <- as.factor(int_effect$Skimmer_Dragonfly)
abundance.df$Skimmer_Dragonfly <- as.factor(abundance.df$Skimmer_Dragonfly)

int_effect.plot <- ggplot( int_effect, aes( MaxTemp, predicted, colour = Skimmer_Dragonfly ) ) +
  geom_line() +
  geom_ribbon(aes( ymin = conf.low, ymax = conf.high ), alpha = 0.25, show.legend = F, colour = NA) +
  geom_point(abundance.df, mapping = aes(x = MaxTemp_Predicted_C, y = Mosquito ), size = 0.75) +
  theme_classic( base_size = 11, base_line_size = 0.1 ) +
  facet_wrap(~Skimmer_Dragonfly) +
  scale_y_continuous( expand = c(0, 0) ) +
  scale_x_continuous( expand = c(0, 0) ) +
  coord_cartesian(ylim = c(0, 500)) +
  xlab("Max Temperature (°C)") +
...
ggsave("../abundance/newplot2.png", plot = int_effect.plot, units = c( "mm" ), dpi = 300, bg = "white" )
```
