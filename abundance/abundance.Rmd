...
int_effect.plot <- ggplot( int_effect, aes( MaxTemp, predicted, colour = drgn_presence ) ) +
  geom_line() +
  geom_ribbon(aes( ymin = conf.low, ymax = conf.high, fill = drgn_presence ), alpha = 0.25, show.legend = F, colour = NA) +
  geom_point(abundance.df, mapping = aes(x = MaxTemp_Predicted_C, y = Mosquito ), size = 1.5, alpha = 0.35 ) +
  theme_classic( base_size = 12, base_line_size = 0.1 ) +
  scale_y_continuous( expand = c(0, 0) ) +
  scale_x_continuous( expand = c(0, 0) ) +
  coord_cartesian(ylim = c(-1, 250), xlim = c(23.8, 36.5)) +
...
ggsave("../abundance/newplot3.png", plot = int_effect.plot, units = c( "mm" ), dpi = 300, bg = "white" )
```
