...
# Technically this has lower AIC than model 4, but the quadratic term isn't significant and the dragonfly effect disappears
mosq.cand.models[[8]] <- glmmTMB(Mosquito ~ I(MaxTemp_Z^2) + MaxTemp_Z * Skimmer_Z + (1 | Date), ziformula = ~., family = truncated_nbinom2(), data = abundance.df)

aic_table <- aictab(cand.set = mosq.cand.models, sort = TRUE)
# write.csv(aic_table, file = "../abundance/aic_table.csv")

summary(mosq.cand.models[[8]])
...
mosq_effect.1 <- effect_plot(mosq.cand.models[[8]], pred = MaxTemp_Z, interval = TRUE, plot.points = TRUE, y.label = "Mosquito Abundance", x.label = "Max Temperature (°C)", point.size = 3) +  theme_classic( base_size = 24, base_line_size = 1 ) + 
...
# ggplot(data = abundance.df, aes(x = MaxTemp_Predicted_C, y = Mosquito, colour = drgn_presence)) +
#   geom_point() +
#   geom_smooth()
```
