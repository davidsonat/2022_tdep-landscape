...
```{r}
# int_effect <- ggpredict(model.avg(abundance.dredge, subset = delta <= 2, fit = TRUE), terms = c("MaxTemp_Z [all]", "drgn_presence"), ci.lvl = 0.95, type = "fe.zi")
# int_effect2 <- ggpredict(model.avg(abundance.dredge, subset = delta <= 2, fit = TRUE), terms = c("drgn_presence"), ci.lvl = 0.95, type = "zi.prob") # Remember, these are pr(ABSENCE)
