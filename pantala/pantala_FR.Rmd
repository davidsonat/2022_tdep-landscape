...
write.csv( Pan_Summary.df, "../pantala/Output_PanSpp.csv" )
```
...
# modelout$Predator <- c("Present")
# modelout2 <- read.csv("../pantala/Output_NoPred.csv")
# modelout2$Predator <- c("Absent")
# modelout <- rbind(modelout, modelout2)
# modelout$Predator <- as.factor(modelout$Predator)
# 
# aggregate(modelout$num_survive, by = list(modelout$temp, modelout$Predator), FUN = mean)
# modelout[is.na(modelout)] = 0
# 
# # modelfig <- ggplot( modelout, aes( x = temp, y = num_survive, colour = Predator ) ) +
...
modelfig <- ggplot( modelout, aes( x = temp, y = num_survive ) ) +
  geom_smooth( size = 0.35, colour = "#00BFC4" ) +
  scale_x_continuous( expand = c( 0, 0 ), breaks = c( 20, 24, 28, 32, 36 ) ) +
  scale_y_continuous( expand = c( 0, 0 ) ) +
  coord_cartesian( ylim = c(0, 100)) +
