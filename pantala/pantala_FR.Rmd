...
modelout3 <- read.csv("../pantala/Output_PanSppNoMort.csv")

modelout$propdiff <- (modelout$num_survive - modelout2$num_survive)/(modelout2$num_survive)
modelout$propsurv <- modelout$num_survive/300
modelout$rawdiff <- modelout$num_survive - modelout2$num_survive

modelfig <- ggplot( modelout, aes( x = temp, y = propsurv ) ) +
  geom_smooth( size = 0.35, linetype = 2, colour = "#00BFC4" ) +
