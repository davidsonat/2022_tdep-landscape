...
tdep.plot <- ggplot( data.frame( Temp = c( 18, 38 ) ), aes( Temp ) ) +
  geom_point( data = frdat, 
              aes( x = Temp, y = Killed, colour = N0, shape = N0 ) ) +
  geom_line( data = tdepfr_out.dat, aes( x = Temp , y = Killed, colour = N0, linetype = N0 ) ) +
  ylab( "Prey Killed" ) +
  xlab( "Temperature (°C)" ) +
  scale_y_continuous( expand = c( 0, 0 ), limits = c( 0, 65 ) ) +
  scale_x_continuous( limits = c( 18, 38 ), breaks = c( 20, 24, 28, 32, 36 ) ) +
  scale_color_manual( "Density", values = c( "black", "black", "black", "black", "black" ), labels = c( "20", "40", "60", "80", "120" ) ) +
  scale_linetype_manual( "Density", values = c( "solid", "longdash", "twodash", "dashed", "dotdash" ), labels = c( "20", "40", "60", "80", "120" ) ) +
  scale_shape_manual( "Density", values = c( 16, 15, 17, 18, 1 ), labels = c( "20", "40", "60", "80", "120" ) ) +
  theme_classic( base_size = 24, base_line_size = 1 ) + 
  theme( panel.border = element_rect( color = "black", fill = NA, size = 0.75 ),
         aspect.ratio = 1 )
...
# Old Panel D -- model out
# modelout.df <- read.csv( "Output_PanSpp.csv" )
# 
...
#          aspect.ratio = 1) + 
#   xlab( "Temperature (°C)" ) +
#   ylab( "Predicted Prop. Survival" )


# Fig3.plot <- plot_grid(Fig3a.plot, Fig3b.plot, Fig3c.plot, Fig3d.plot, ncol = 2, nrow = 2, align = c("v"), axis = c("l"), labels = c("a)", "b)", "c)", "d)"), label_x = 0.2 )
# ggsave("Fig3.png", plot = Fig3.plot, units = c( "mm" ), dpi = 300, bg = "white" )
